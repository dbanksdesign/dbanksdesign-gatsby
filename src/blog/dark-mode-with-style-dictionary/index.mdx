...
We are going to build a design token package using Style Dictionary that has support for light and dark mode for color tokens and graphic assets (using tokens as well). It will have demo apps in Android, iOS, and Web to show how it would integrate into those platforms. This article will serve as a high-level explanation of concepts; the why and how things work. The [Github repository](repo) has everything fully implemented with comments and more in-depth explanations. 

This article and sample will not cover:
* How to think about and build dark mode support from a UX perspective (like building a setting page or control for toggling light/dark mode)
* Non-platform or framework-specific ways of implementing light/dark mode. For example, using a `<ThemeProvider>` in React or using a CSS-in-JS framework. You can absolutely still use those things if you want. I chose not to include every possible integration and instead stick to the native web, Android, and iOS mechanisms for light/dark mode.
...
* A "complete" design token package or design system. To keep it short this will focus only on color tokens with different tiers, which will be explained later, and graphic assets as design tokens. I only built 2 very components: button, and badge with limited options just to illustrate how the color tokens work at a component level. 
...
Before we start, let's look at how different platforms handle color modes. If you don't care about this stuff and want to just jump into the setup you can skip these sections.
...
Using CSS custom properties and having a layer of semantic tokens allows you to not have to re-style every component for light and dark mode. The above example is very simple, but can be expanded to a fit the needs of a full design system. Also if you are using inline SVG (as opposed to using SVG in an `<img>` tag), your SVG can be styled with CSS and can use CSS custom properties as well! One more quick tip before we move on: you can use the `currentColor` property in SVG as well to have the graphic inherit its color from its parent. This is useful if you have an icon inside a link or button for example. 
...
[See this MDN article for more information about the prefers-color-scheme media query](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-color-scheme)
