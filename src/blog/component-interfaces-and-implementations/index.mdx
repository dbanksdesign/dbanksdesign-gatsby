...
Taken to an extreme, this can cause issues if you define components so strictly they do not make sense for other areas of your application to use. For example we could define the chart above as a `TrafficChart`, but that would severly constrain the scenarios it can be used for. 
...
* Use semantic abstractions like 'title' and 'subtitle' to make the API generic, but semantic.
* Use abstract theme or variation attributes, such as: danger, warning, success.
